...
1. 함수가 실행되기 전에 [[실행 컨텍스트(Execution Context)|실행 컨텍스트]]를 생성하는 과정에서 선언 단계와 초기화 단계가 함께 일어난다.
...
1. 함수가 실행되기 전에 [[실행 컨텍스트(Execution Context)|실행 컨텍스트]]를 생성하는 과정에서 선언 단계가 일어난다.
...
```js
function createTDZ(a=b, b) { // ReferenceError: Cannot access 'b' before initialization
}
...
let a = f(); // 1
const b = 2;
function f() { return b; } // ReferenceError: b is not defined
```
