...
## [[클래스(Class)|클래스 문법]]은 단지 "[[구문적 설탕(Syntatic sugar)]]" 인가?

- 클래스 문법을 사용하는 것과 비슷한 결과를 기존 function + prototype 문법의 조합으로 만들 수 있기 때문에 일부 사람들은 단지 클래스 문법을 "[[구문적 설탕(Syntatic sugar)|구문적 설탕]]"이라고 말한다.
- 하지만 몇 가지 중요한 차이가 있다.
	...
	- 클래스의 메소드는 [[enumerable(열거 가능)|non-enumerable]] 로, [[for ... of vs for ... in|for ... in]] 구문 등의 객체 열거에서 메서드는 열거하지 않는다. ([[JavaScript 객체(Object)|enumerable 플래그]]가 false 이다.)
	- 클래스는 항상 [[엄격 모드(strict mode)|use strict 모드]]이다.
