...
3. 링크를 누르고 HTML 문서가 열렸을 때, 아래와 같은 코드로 공격 대상 사이트에 HTTP 요청을 보냄.
	 ```html
	<img src="https://site-a.com/withdraw?account=bob&amount=1000000&for=mallory">
	 ```
...
2. 민감한 작업(ex. 세션 쿠키)에 사용되는 쿠키의 [[동일 사이트(Same Site)|SameSite]] 속성을 Strict 또는 Lax로 설정하고, 짧은 수명을 가지게 한다.
3. 위의 두 방법을 모두 사용해야 한다.
	- [[동일 사이트(Same Site)|SameSite]] 속성은 하위 도메인에서 발생하는 공격에 도움이 되지 않는다.
