...
- 일반적으로 빠른 [[FP vs FCP vs FMP vs LCP|FP 및 FCP]]
...
- 빠른 [[FP vs FCP vs FMP vs LCP|FP 및 FCP]] 그러나, [[TTI(Time To Interactive)]]와 간극이 길어질 수 있다.
- 서버에서 페이지를 생성하는데 시간이 걸려 [[TTFB(Time To First Byte)]]가 느려질 수 있다.
- [[처리량(throughput)]]이 낮다.
...
- 빠른 [[FP vs FCP vs FMP vs LCP|FP 및 FCP]]
- 빠른 [[TTI(Time To Interactive)]]
- 서버 렌더링과 달리 HTML을 즉석에서 생성하지 않아도 되므로 빠른 [[TTFB(Time To First Byte)]]를 가진다.
	- 정적 렌더링을 여러 [[CDN(Content Delivery Network)]]에 배포하여 캐싱할 수 있다.
...
- 서버에서 HTML을 생성하지 않으므로 [[TTFB(Time To First Byte)]]가 빠르다.
- JavaScript의 양이 증가한다.
- [[처리량(throughput)]]이 높다.
- 빈 흰 화면이 오래 지속될 수 있다.
- [[검색 엔진 최적화(SEO)|SEO]] 측면에서 제한 사항이 있다.
